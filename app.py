...
async def query(req: QueryRequest):
    ...
    try:
        # Step 2: Ask LLM to interpret and generate SQL
        ...
        try:
            query_result = db.execute_query(sql, llm_response.get("sql_params"))
            ...
            if not sql:
                ...
                history.append({"role": "assistant", "content": answer})
                return QueryResponse(answer=answer, sql=llm_response.get("sql"), error=error_msg)

            logger.info("Retry SQL: %s", sql)
            try:
                query_result = db.execute_query(sql, retry_response.get("sql_params"))
        ...
        return QueryResponse(answer=error_answer, error=str(e))
...
async def export_csv(request: Request):
    """Export query result data as a CSV file."""
